...
# gsutil is no longer very reliable for S3 so it is no longer 
# recommended
#
...
function snapshot_cleanup {
  ...
  SNAPSHOT_LIST=$($AWSCLI s3 ls s3://"${BUCKET}"/"${REMOTEDIR}""${SNAPSHOT_ROTATION_SUFFIX}"|sort|uniq|sort)
  ...
  do
    ...
    if ! $AWSCLI s3 rm "${REMOVEFILE}"; then
      >&2 echo "ERROR: $0, Could not perform snapshot cleanup. Check your permissions."
      return
    fi

    SNAPSHOT_LIST=$($AWSCLI s3 ls "${PROTOCOL}"://"${BUCKET}"/"${REMOTEDIR}"|sort|uniq|sort)
    SNAPSHOT_COUNT=$(echo "${SNAPSHOT_LIST}"|wc -l)
  done
}
...
function gsutil_check {
    if ! $AWSCLI s3 ls s3://"${BUCKET}" > /dev/null; then
...
function push_snapshot {
  ...
  /bin/dd if="/dev/${VOLGROUP}/${SNAPNAME}" bs=128k status=none|\
     ...
     ${AWSCLI} s3 cp - s3://"${BUCKET}"/"${REMOTEDIR}"/"${SNAPNAME}"-"$(date +%Y%m%d-%H%M.dd.gz.gpg)"
